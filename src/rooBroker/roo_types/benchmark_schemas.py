...
class StringContainsTestCase(BaseTestCase):
    ...
    def validate_expected_string(cls, v):
        if not isinstance(v, str):
            raise ValueError("Expected value must be a string for string_contains test cases")
        return v
...
class ExecCheckStateTestCase(BaseTestCase):
    ...
    def validate_expected_dict(cls, v):
        if not isinstance(v, dict):
            raise ValueError("Expected value must be a dictionary for exec_check_state test cases")
        return v
