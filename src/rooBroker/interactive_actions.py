...
async def manual_save_state(layout: InteractiveLayout, benchmark_results: List[Dict[str, Any]]):
...
async def view_benchmark_results(layout: InteractiveLayout):
    ...
    try:
        # Load model state
        model_list = await asyncio.to_thread(load_models_as_list, ".modelstate.json", layout.console)
...
async def launch_context_proxy(layout: InteractiveLayout):
    ...
    try:
        if proxy_server:
            layout.prompt.add_message("[yellow]Proxy server is already running.[/yellow]")
            return
...
async def discover_models_only(layout: InteractiveLayout, discovered_models_arg: List[DiscoveredModel]):
    ...
    try:
        ...
        if status["total_count"] > 0:
            ...
            layout.prompt.add_message(f"[green]Discovered {status['total_count']} models.[/green]")
...
async def run_benchmarks_with_config(layout: InteractiveLayout, app_state: Dict[str, Any], benchmark_results: List[Dict[str, Any]], discovered_models: List[DiscoveredModel]):
    ...
    if model_source == 'discovered':
        ...
        try:
            raw_models = await asyncio.to_thread(load_models_as_list, ".modelstate.json", layout.console)
            for raw_model in raw_models:
                if "id" in raw_model:
                    if "family" in raw_model:
                        models_to_run.append(cast(DiscoveredModel, ModelInfo(id=raw_model["id"], family=raw_model.get("family", ""))))
        ...
        ids_str = layout.console.input("Enter model IDs to benchmark (comma-separated): ")
...
async def run_all_steps(layout: InteractiveLayout, app_state: Dict[str, Any], discovered_models: List[DiscoveredModel], benchmark_results: List[Dict[str, Any]]):
    ...
    # 2. Benchmark models
    await run_benchmarks_with_config(layout, app_state, benchmark_results, discovered_models)
...
async def handle_benchmark_option(option: str, layout: InteractiveLayout, app_state: Dict[str, Any], benchmark_results: List[Dict[str, Any]], discovered_models: List[DiscoveredModel]):
